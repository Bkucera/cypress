  ...
  context("#type", function() {
    ...
    describe("value changing", function() {
      ...
      describe("[contenteditable]", function() {
        ...
        it("collapses selection to start on {leftarrow}", function() {
          ...
          return cy.get("[contenteditable]:first")
          .type("{selectall}{leftarrow}foo").then($div => expect($div.get(0).innerText).to.eql("foobar\n"));
        });

        it("collapses selection to end on {rightarrow}", function() {
          cy.$$('[contenteditable]:first').get(0).innerHTML = '<div>bar</div>';
          return cy.get("[contenteditable]:first")
          .type("{selectall}{leftarrow}foo{selectall}{rightarrow}baz").then($div => expect($div.get(0).innerText).to.eql("foobarbaz\n"));
        });
    ...
    describe("specialChars", function() {
      ...
      context("{uparrow}", function() {
        ...
        it("up and down arrow on contenteditable", function() {
          ...
          return cy.get("[contenteditable]:first")
          .type("{leftarrow}{leftarrow}{uparrow}11{uparrow}22{downarrow}{downarrow}33").then($div => expect($div.get(0).innerText).to.eql("foo22\nb11ar\nbaz33\n"));
        });
        ...
        it("uparrow ignores current selection", function() {
          ...
          return cy.get("[contenteditable]:first")
          .type("{uparrow}11").then($div => expect($div.get(0).innerText).to.eql("11foo\nbar\nbaz\n"));
        });
      ...
      context("{downarrow}", function() {
        ...
        return it("downarrow ignores current selection", function() {
          ...
          return cy.get("[contenteditable]:first")
          .type("{downarrow}22").then($div => expect($div.get(0).innerText).to.eql("foo\n22bar\nbaz\n"));
        });
      });
    ...
    describe("modifiers", function() {
      ...
      return describe("changing modifiers", function() {
        ...
        it("sends keydown event for new modifiers", function() {
          ...
          return cy.get("input:text:first").type("{shift}").then(() => expect(spy).to.be.calledWithMatch({which: 16}));
        });
        ...
        return it("does not send keydown event for already activated modifiers", function() {
          ...
          return cy.get("input:text:first").type("{cmd}{alt}").then(() => expect(spy).to.not.be.called);
        });
      });
    });
    ...
    describe("caret position", function() {
      ...
      it("can wrap cursor to next line in [contenteditable] with {rightarrow}", function() {
        ...
        .type('{rightarrow}'.repeat(14)+'[_I_]').then(() => expect(cy.$$('[contenteditable]:first').get(0).innerText).to.eql('start\nmiddle\ne[_I_]nd\n'));
      });
      ...
      it("can wrap cursor to prev line in [contenteditable] with {leftarrow}", function() {
        ...
        '<div>middle</div>'+
        '<div>end</div>';
        return cy.get('[contenteditable]:first').type('{leftarrow}'.repeat(12)+'[_I_]').then(() => expect(cy.$$('[contenteditable]:first').get(0).innerText).to.eql('star[_I_]t\nmiddle\nend\n'));
      });
      ...
      it("can use {rightarrow} and nested elements", function() {
        ...
        return cy.get('[contenteditable]:first')
        .type('{selectall}{leftarrow}')
        .type('{rightarrow}'.repeat(3)+'[_I_]').then(() => expect(cy.$$('[contenteditable]:first').get(0).innerText).to.eql('sta[_I_]rt\n'));
      });

      it("enter and \\n should act the same for [contenteditable]", function() {

        const cleanseText = text => text.replace(/ /g, ' ');

        const expectMatchInnerText = ($el , innerText) => expect(cleanseText($el.get(0).innerText)).to.eql(innerText);
    ...
    describe("{enter}", function() {
      ...
      context("2 inputs, no 'submit' elements", () =>
        it("does not trigger submit event", function(done) {
          const form = this.$forms.find("#no-buttons").submit(() => done("err: should not have submitted"));

          return cy.get("#no-buttons input:first").type("f").type("{enter}").then(() => done());
        })
      );
